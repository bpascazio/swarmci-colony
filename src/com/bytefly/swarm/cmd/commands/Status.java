		...
		if (uid != 0) {
			...
			for (int i = 0; i < pl.cv.size(); i++) {
				...
				if (p.buildNum == 0)
					...
					bld = "" + p.buildNum;
				System.out.print(p.Name + " build-number[" + bld + "] status["
						+ ((p.buildState == 1) ? "ON" : "OFF") + "] "
						+ ((p.buildTrigger == 1) ? "RUNNING " : "") + "repo["
						+ p.Repo + "] builder[" + p.Builder + "]\n");
			}
		} else {
			System.out
					.print("Could not connect to server or failure to authenticate.\n");
		}
	}
